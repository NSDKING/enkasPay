...
export default function SitchiUpdateForm(props) {
  const {
    id: idProp,
    sitchi: sitchiModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = sitchiRecord
      ? { ...initialValues, ...sitchiRecord }
      : initialValues;
    setNom(cleanValues.Nom);
    setPrenom(cleanValues.Prenom);
    setNumero(cleanValues.numero);
    setCite(cleanValues.cite);
    setPrecommender(cleanValues.precommender);
    setErrors({});
  };
  const [sitchiRecord, setSitchiRecord] = React.useState(sitchiModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Sitchi, idProp)
        : sitchiModelProp;
      setSitchiRecord(record);
    };
    queryData();
  }, [idProp, sitchiModelProp]);
  React.useEffect(resetStateValues, [sitchiRecord]);
  const validations = {
    Nom: [],
    Prenom: [],
    numero: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Sitchi.copyOf(sitchiRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "SitchiUpdateForm")}
      {...rest}
    >
      <TextField
        label="Nom"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || sitchiModelProp)}
